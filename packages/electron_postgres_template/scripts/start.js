...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(
    __dirname,
    "../lib/node_modules/electron_postgres_template",
  );
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/electron_postgres_template");
  }
}
...
if (
  ...
  isTemp = true;
  workDir = join(tmpdir(), `electron_postgres_template-${Date.now()}`);
