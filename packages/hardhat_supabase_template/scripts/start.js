...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(
    __dirname,
    "../lib/node_modules/hardhat_supabase_template",
  );
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/hardhat_supabase_template");
  }
}
...
if (
  ...
  isTemp = true;
  workDir = join(tmpdir(), `hardhat_supabase_template-${Date.now()}`);
