...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(__dirname, "../lib/node_modules/astro_supabase_template");
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/astro_supabase_template");
  }
}
...
if (packageRoot.startsWith("/nix/store")) {
  isTemp = true;
  workDir = join(tmpdir(), `astro_supabase_template-${Date.now()}`);
...
if (process.env.DEBUG === "1") {
  ...
  const buildCmd = "";
  const startCmd = "npm run dev -- --port 3000";
  if (isTemp) {
    if (setupCmd) {
      fullCmd += `${setupCmd} && `;
    }
