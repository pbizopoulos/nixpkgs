defmodule PhoenixAppWeb.UserAuthTest do
  use PhoenixAppWeb.ConnCase, async: true
  alias Phoenix.LiveView
  alias PhoenixApp.Accounts
  alias PhoenixApp.Accounts.Scope
  alias PhoenixAppWeb.UserAuth
  import PhoenixApp.AccountsFixtures
  @remember_me_cookie "_phoenix_app_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, PhoenixAppWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      PhoenixAppWeb.Endpoint.subscribe(live_socket_id)
      conn
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        PhoenixAppWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
