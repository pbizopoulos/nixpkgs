...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(__dirname, "../lib/node_modules/qwik_supabase_template");
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/qwik_supabase_template");
  }
}
...
if (packageRoot.startsWith("/nix/store")) {
  isTemp = true;
  workDir = join(tmpdir(), `qwik_supabase_template-${Date.now()}`);
