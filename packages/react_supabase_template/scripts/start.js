...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(__dirname, "../lib/node_modules/react_supabase_template");
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/react_supabase_template");
  }
}
...
if (
  ...
  isTemp = true;
  workDir = join(tmpdir(), `react_supabase_template-${Date.now()}`);
