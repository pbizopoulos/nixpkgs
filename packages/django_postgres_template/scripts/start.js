...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(__dirname, "../lib/node_modules/django_postgres_template");
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/django_postgres_template");
  }
}
...
if (packageRoot.startsWith("/nix/store")) {
  isTemp = true;
  workDir = join(tmpdir(), `django_postgres_template-${Date.now()}`);
...
if (process.env.DEBUG === "1") {
  ...
  let fullCmd = "";
  const setupCmd = "pip install -r requirements.txt";
  const buildCmd = "";
  const startCmd = "python manage.py runserver 0.0.0.0:3000";
  if (isTemp) {
    if (setupCmd) {
      fullCmd += `${setupCmd} && `;
    }
