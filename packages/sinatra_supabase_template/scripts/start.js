...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(
    __dirname,
    "../lib/node_modules/sinatra_supabase_template",
  );
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/sinatra_supabase_template");
  }
}
...
if (packageRoot.startsWith("/nix/store")) {
  isTemp = true;
  workDir = join(tmpdir(), `sinatra_supabase_template-${Date.now()}`);
...
if (process.env.DEBUG === "1") {
  ...
  const buildCmd = "";
  const startCmd = "bundle exec ruby app/app.rb -o 0.0.0.0";
  if (isTemp) {
    if (setupCmd) {
      fullCmd += `${setupCmd} && `;
    }
