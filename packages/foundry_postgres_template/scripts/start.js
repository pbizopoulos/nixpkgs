...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(
    __dirname,
    "../lib/node_modules/foundry_postgres_template",
  );
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/foundry_postgres_template");
  }
}
...
if (
  ...
  isTemp = true;
  workDir = join(tmpdir(), `foundry_postgres_template-${Date.now()}`);
