...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(
    __dirname,
    "../lib/node_modules/fastapi_postgres_template",
  );
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/fastapi_postgres_template");
  }
}
...
if (packageRoot.startsWith("/nix/store")) {
  isTemp = true;
  workDir = join(tmpdir(), `fastapi_postgres_template-${Date.now()}`);
