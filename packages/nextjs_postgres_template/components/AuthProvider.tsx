...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const postgresKey = process.env.NEXT_PUBLIC_POSTGRES_ANON_KEY || "";
  const [postgres] = useState(() =>
    createBrowserClient(postgresUrl, postgresKey, {
      global: {
        fetch: (url, options) => {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async (userId: string) => {
      const { data: profileData } = await postgres
        .from("users")
        .select("username")
        .eq("auth_id", userId)
        .maybeSingle();
    ...
    const {
      data: { subscription },
    } = postgres.auth.onAuthStateChange(async (_event, session) => {
