...
describe("AuthForm", () => {
  ...
  it("should validate username in sign up mode", async () => {
    ...
    expect(mockPostgres.auth.signUp).not.toHaveBeenCalled();
  });
  it("should handle successful sign up", async () => {
    vi.mocked(mockPostgres.auth.signUp).mockResolvedValue({
  ...
  it("should handle successful sign in", async () => {
    vi.mocked(mockPostgres.auth.signInWithPassword).mockResolvedValue({
      data: { session: {} },
      error: null,
    });
    render(<AuthForm />);
