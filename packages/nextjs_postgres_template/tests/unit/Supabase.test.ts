...
import { createClient, getAuthenticatedUser } from "@/lib/postgres";
...
describe("Postgres", () => {
  ...
  it("should create a client with cookies by default", async () => {
    process.env.NEXT_PUBLIC_POSTGRES_URL = "https://example.com";
    process.env.NEXT_PUBLIC_POSTGRES_ANON_KEY = "anon-key";
    vi.mocked(cookies).mockResolvedValue(mockCookieStore as any);
    const postgresClient = (await createClient()) as any;
    expect(postgresClient).toBeDefined();
    expect(postgresClient.url).toBe("https://example.com");
    expect(postgresClient.key).toBe("anon-key");
    expect(postgresClient.options.cookies).toBeDefined();
    expect(postgresClient.options.cookies.getAll).toBeDefined();
    expect(postgresClient.options.cookies.getAll()).toEqual([]);
    expect(postgresClient.options.cookies.setAll).toBeDefined();
  });
  it("should create a client without cookies when useCookies is false", async () => {
    process.env.NEXT_PUBLIC_POSTGRES_URL = "https://example.com";
    process.env.NEXT_PUBLIC_POSTGRES_ANON_KEY = "anon-key";
    const postgresClient = (await createClient(false)) as any;
    expect(postgresClient).toBeDefined();
    expect(postgresClient.options.cookies.getAll()).toEqual([]);
    postgresClient.options.cookies.setAll([
  ...
  it("should throw error if env vars are missing", async () => {
    ...
    await expect(createClient()).rejects.toThrow(
      "Missing env NEXT_PUBLIC_POSTGRES_URL",
    );
    process.env.NEXT_PUBLIC_POSTGRES_URL = "https://example.com";
    process.env.NEXT_PUBLIC_POSTGRES_ANON_KEY = "";
    await expect(createClient()).rejects.toThrow(
      "Missing env NEXT_PUBLIC_POSTGRES_ANON_KEY",
    );
  });
  it("should test the custom fetch implementation", async () => {
    ...
    const postgresClient = (await createClient()) as any;
    const customFetch = postgresClient.options.global.fetch;
