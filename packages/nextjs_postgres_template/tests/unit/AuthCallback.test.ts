...
describe("Auth Callback route", () => {
  ...
  it("should test the custom fetch and cookie handling in createServerClient", async () => {
    vi.stubEnv("NEXT_PUBLIC_POSTGRES_URL", "http://localhost:54321");
    vi.stubEnv("NEXT_PUBLIC_POSTGRES_ANON_KEY", "key");
    vi.mocked(mockPostgres.auth.exchangeCodeForSession).mockResolvedValue({
      error: null,
    });
