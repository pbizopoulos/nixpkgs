...
describe("AuthProvider", () => {
  ...
  beforeEach(() => {
    ...
    mockPostgres.auth.onAuthStateChange.mockReturnValue({
      data: { subscription: { unsubscribe: vi.fn() } },
    });
    mockPostgres.from.mockReturnValue({
  ...
  it("should update state on auth change", async () => {
    let authStateCallback: ((event: string, session: unknown) => void) | null =
      null;
    mockPostgres.auth.onAuthStateChange.mockImplementation(
  ...
  it("should handle error during initialization", async () => {
    ...
    mockPostgres.auth.getUser.mockRejectedValue(new Error("Init failed"));
    await act(async () => {
      render(
        <AuthProvider>
          <TestComponent />
  ...
  it("should sign out", async () => {
    ...
    expect(mockPostgres.auth.signOut).toHaveBeenCalled();
    await waitFor(() => {
      expect(screen.getByTestId("user").textContent).toBe("No User");
    });
  });
  ...
  it("should handle unmount during initUser", async () => {
    ...
    mockPostgres.auth.getUser.mockReturnValue(
      new Promise((resolve) => {
  ...
  it("should handle env variable fallbacks", async () => {
    vi.stubEnv("NEXT_PUBLIC_POSTGRES_URL", "");
    vi.stubEnv("NEXT_PUBLIC_POSTGRES_ANON_KEY", "");
    await act(async () => {
      render(
        <AuthProvider>
          <div>Test</div>
  ...
  it("should handle throw in initUser catch block", async () => {
    const consoleSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    mockPostgres.auth.getUser.mockImplementation(() => {
