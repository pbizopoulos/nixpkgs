...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(__dirname, "../lib/node_modules/blitzjs_template");
}
let workDir = packageRoot;
let isTemp = false;
if (!existsSync(join(packageRoot, "node_modules"))) {
  isTemp = true;
  workDir = join(tmpdir(), `blitzjs_template-${Date.now()}`);
...
if (process.env.DEBUG === "1") {
  console.log("Smoke testing Blitz.js App...");
  ...
  const blitz = spawn(`${setup}npm run start`, [], {
    stdio: "inherit",
    cwd: workDir,
    shell: true,
  });
