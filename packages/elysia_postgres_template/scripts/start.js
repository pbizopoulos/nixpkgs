...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(__dirname, "../lib/node_modules/elysia_postgres_template");
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/elysia_postgres_template");
  }
}
...
if (packageRoot.startsWith("/nix/store")) {
  isTemp = true;
  workDir = join(tmpdir(), `elysia_postgres_template-${Date.now()}`);
...
if (process.env.DEBUG === "1") {
  ...
  let fullCmd = "";
  const setupCmd = "bun install";
  const buildCmd = "";
  const startCmd = "bun run src/index.ts";
  if (isTemp) {
    if (setupCmd) {
      fullCmd += `${setupCmd} && `;
    }
