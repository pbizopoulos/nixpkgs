...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(__dirname, "../lib/node_modules/nestjs_supabase_template");
}
let workDir = packageRoot;
let isTemp = false;
if (!existsSync(join(packageRoot, "node_modules"))) {
  isTemp = true;
  workDir = join(tmpdir(), `nestjs_supabase_template-${Date.now()}`);
...
if (process.env.DEBUG === "1") {
  console.log("Smoke testing NestJS App...");
  ...
  const nuxt = spawn(`${setup}npm run dev`, [], {
    stdio: "inherit",
    cwd: workDir,
    shell: true,
    env: { ...process.env, NUXT_TELEMETRY_DISABLED: "1" },
