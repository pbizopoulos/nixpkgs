...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(
    __dirname,
    "../lib/node_modules/redwoodjs_postgres_template",
  );
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/redwoodjs_postgres_template");
  }
}
...
if (
  ...
  isTemp = true;
  workDir = join(tmpdir(), `redwoodjs_postgres_template-${Date.now()}`);
