...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(
    __dirname,
    "../lib/node_modules/electron_supabase_template",
  );
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/electron_supabase_template");
  }
}
...
if (packageRoot.startsWith("/nix/store")) {
  isTemp = true;
  workDir = join(tmpdir(), `electron_supabase_template-${Date.now()}`);
