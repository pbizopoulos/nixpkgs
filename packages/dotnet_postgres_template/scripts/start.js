...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(__dirname, "../lib/node_modules/dotnet_postgres_template");
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/dotnet_postgres_template");
  }
}
...
if (packageRoot.startsWith("/nix/store")) {
  isTemp = true;
  workDir = join(tmpdir(), `dotnet_postgres_template-${Date.now()}`);
...
if (process.env.DEBUG === "1") {
  ...
  let fullCmd = "";
  const setupCmd = "";
  const buildCmd = "dotnet build";
  const startCmd = "dotnet run --project app";
  if (isTemp) {
    if (setupCmd) {
      fullCmd += `${setupCmd} && `;
    }
