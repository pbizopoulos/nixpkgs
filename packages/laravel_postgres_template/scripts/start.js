...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(
    __dirname,
    "../lib/node_modules/laravel_postgres_template",
  );
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/laravel_postgres_template");
  }
}
...
if (
  ...
  isTemp = true;
  workDir = join(tmpdir(), `laravel_postgres_template-${Date.now()}`);
...
if (process.env.DEBUG === "1") {
  ...
  let fullCmd = "";
  const setupCmd = "composer install";
  const buildCmd = "";
  const startCmd = "php artisan serve --host 0.0.0.0 --port 3000";
  if (isTemp) {
    if (setupCmd) {
      fullCmd += `${setupCmd} && `;
    }
