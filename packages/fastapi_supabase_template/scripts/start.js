...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(
    __dirname,
    "../lib/node_modules/fastapi_supabase_template",
  );
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/fastapi_supabase_template");
  }
}
...
if (
  ...
  isTemp = true;
  workDir = join(tmpdir(), `fastapi_supabase_template-${Date.now()}`);
