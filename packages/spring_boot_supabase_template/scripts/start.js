...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(
    __dirname,
    "../lib/node_modules/spring_boot_supabase_template",
  );
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/spring_boot_supabase_template");
  }
}
...
if (
  ...
  isTemp = true;
  workDir = join(tmpdir(), `spring_boot_supabase_template-${Date.now()}`);
