...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(__dirname, "../lib/node_modules/tanstack_supabase_template");
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/tanstack_supabase_template");
  }
}
...
if (packageRoot.startsWith("/nix/store")) {
  isTemp = true;
  workDir = join(tmpdir(), `tanstack_supabase_template-${Date.now()}`);
