...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(__dirname, "../lib/node_modules/rails_postgres_template");
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/rails_postgres_template");
  }
}
...
if (
  ...
  isTemp = true;
  workDir = join(tmpdir(), `rails_postgres_template-${Date.now()}`);
...
if (process.env.DEBUG === "1") {
  ...
  let fullCmd = "";
  const setupCmd = "bundle install";
  const buildCmd = "";
  const startCmd = "bin/rails server -b 0.0.0.0 -p 3000";
  if (isTemp) {
    if (setupCmd) {
      fullCmd += `${setupCmd} && `;
    }
