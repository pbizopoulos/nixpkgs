...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(__dirname, "../lib/node_modules/solidstart_template");
}
let workDir = packageRoot;
let isTemp = false;
if (!existsSync(join(packageRoot, "node_modules"))) {
  isTemp = true;
  workDir = join(tmpdir(), `solidstart_template-${Date.now()}`);
...
if (process.env.DEBUG === "1") {
  console.log("Smoke testing SolidStart App...");
  ...
  const solid = spawn(`${setup}npm run start`, [], {
    stdio: "inherit",
    cwd: workDir,
    shell: true,
  });
