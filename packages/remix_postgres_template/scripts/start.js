...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(__dirname, "../lib/node_modules/remix_postgres_template");
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/remix_postgres_template");
  }
}
...
if (packageRoot.startsWith("/nix/store")) {
  isTemp = true;
  workDir = join(tmpdir(), `remix_postgres_template-${Date.now()}`);
