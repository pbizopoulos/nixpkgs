...
if (__dirname.endsWith("/bin")) {
  packageRoot = join(
    __dirname,
    "../lib/node_modules/monogame_postgres_template",
  );
  if (!existsSync(packageRoot)) {
    packageRoot = join(__dirname, "../lib/monogame_postgres_template");
  }
}
...
if (
  ...
  isTemp = true;
  workDir = join(tmpdir(), `monogame_postgres_template-${Date.now()}`);
