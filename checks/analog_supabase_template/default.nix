...
let
  mockSupabase = pkgs.writeShellScriptBin "supabase" ''
    ...
    exit 0
  '';
  name = "analog_supabase_template";
  analog-mocked = inputs.self.packages.${pkgs.stdenv.system}.${name}.override {
...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    machine.succeed("cp -r ${analog-mocked}/lib/node_modules/${name} /tmp/${name}")
