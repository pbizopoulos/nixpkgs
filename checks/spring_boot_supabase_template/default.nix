...
let
  ...
  springboot-mocked = inputs.self.packages.${pkgs.stdenv.system}.${name}.override {
...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    machine.succeed("cp -r ${springboot-mocked}/lib/${name} /tmp/${name}")
    machine.succeed("chmod -R +w /tmp/${name}")
    machine.succeed("cd /tmp/${name} && ./gradlew test")
  '';
}
